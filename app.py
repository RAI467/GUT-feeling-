...
def clear_old_updates():
    try:
        updates = bot.get_updates(limit=100, offset=-1)
    except:
        pass
#--------------------
class SimpleAPI:
...
def start_spam():
    ...
    if not target_id:
        return jsonify({"status": "error", "message": "EnTeR The Id user_id"})
...
def telegram_spam(message):
    try:
        ...
        if len(parts) < 2:
            bot.reply_to(message, "PuT The iD!")
            return
        ...
        api.process_spam_command(user_id, duration)

        bot.reply_to(message, "SpAm HaS BeEn [StArTeD✅] 4 This Id => " + user_id)

    except Exception as e:
        bot.reply_to(message, f"SpAm HaS BeEn [StArTeD✅] 4 This Id => {user_id}")
#منع التكرار
...
def telegram_stop(message):
    try:
        ...
        user_id = extract_id(parts)

        if not user_id:
            bot.reply_to(message, "Id WaS NoT pRoViDeD In The CoMMaNd!")
            return

        api.process_stop_command(user_id)

        if user_id in used_ids:
            used_ids.remove(user_id)

        bot.reply_to(message, f"Id ReMoVeD [SuCCeSSfUllY✅] => {user_id}")
...
def telegram_spam(message):
    try:
        ...
        if user_id in used_ids:
            bot.reply_to(message, "This Id Is AlReAdY In The ReQuEsT!")
            return
        ...
        bot.reply_to(message, f"Id AddEd [SuCCeSSfUllY✅] => {user_id}")
...
def check_status(message):
    try:
        ...
        if len(parts) < 2:
            bot.reply_to(message, "ID not provided!")
            return

        user_id = parts[1].replace("/", "").strip()

        status = invite_status.get(user_id)

        if status == "active":
            bot.reply_to(message, f"Id => {user_id} Is In SpAm [StAtUs✅]")
        elif status == "stopped":
            bot.reply_to(message, f"Id => {user_id} Is NoT In SpAm [StAtUs ❎]")
        else:
            bot.reply_to(message, f"Id => {user_id} HaS nO SpAm [ReCoRd ⚠️]")
...
def telegram_stop(message):
    try:
        ...
        bot.reply_to(message, "SpAm HaS BeEn [StOpPeD❎] 4 This Id => " + user_id)

    except Exception as e:
        bot.reply_to(message, f"SpAm HaS BeEn [StOpPeD❎] 4 This Id => {user_id}")
        

ALLOWED_USER = "code_76s"  # بدون @

@bot.message_handler(func=lambda message: message.chat.type == "private")
def private_msg(message):
    username = message.from_user.username

    # إذا مو الأدمن → تجاهل تام (لا رد)
    if username != ALLOWED_USER:
        return

@app.route('/ax', methods=['GET'])
def get_accounts():
    try:
        with connected_clients_lock:
            accounts_count = len(connected_clients)
        ...
        return jsonify({"st": "success", "data": accounts_data})
        
    except Exception as e:
        return jsonify({"status": "error", "message": f" خطاء فالوصول للحسابات {str(e)}"})

#---------(help)---------
@bot.message_handler(commands=['help','start'])
def start_cmd(message):
    bot.send_message(message.chat.id, """WeLcOmE To BoT SAD Spam 

- /sp Uid => SpAm InViTaTioN To Join The RooM

- /x Uid => StOp SpAm InViTaTion To Join The Room

- /st => BoTs StAtUs 

- /vx Uid => ErAsIng The PlAyErs IdEnTity

- /ax => Chk Accs

- /ar => CoMMaNdS In Arabic

---------(DeV,AdN)----------

DeVeLoPr By => @code_76s 
""") 
#-------------(ar)-------------
...
- /sp Uid => إرسال دعوات سبام للانضمام إلى الغرفة

- /x Uid => إيقاف إرسال دعوات السبام للانضمام إلى الغرفة

- /st => حالة البوتات

- /vx Uid => طمس هوية اللاعب
...
تم التطوير بواسطة => @code_75s
""")
#-------(طمس هوية)-------
@bot.message_handler(commands=['vx'])
def erase_identity(message):
    parts = message.text.split()
    if len(parts) == 1:
        bot.reply_to(message, "Please provide the user ID.")
    else:
        user_id = parts[1]
        bot.reply_to(message, f"ErAsIng The PlAyErs IdEnTity 4 Id => {user_id}")
...
class FF_CLient():
    ...
    def ToKen_GeneRaTe(self , Access_ToKen , Access_Uid):
        ...
        self.dT = self.dT.replace(b'2026-04-08 06:55:41' , str(datetime.now())[:-7].encode())        
