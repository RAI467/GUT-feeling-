...
def resolve_via_doh(hostname):
    """Resolve hostname using Cloudflare DNS over HTTPS to bypass HF DNS block"""
    try:
...
print("DNS patch applied: api.telegram.org will be resolved via Cloudflare DoH")


WEBHOOK_URL = os.environ.get('WEBHOOK_URL', 'https://.hf.SADDDFF/spam-bott')
...
def run_bot():
    ...
    if WEBHOOK_URL:
        try:
            ...
            print(f"Failed to set webhook (will still receive updates if pre-configured): {e}")
    else:
        print("No WEBHOOK_URL set, running in passive webhook mode")
    print("Bot is ready to receive webhook updates via Flask")
